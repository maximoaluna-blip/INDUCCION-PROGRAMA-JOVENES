...
- ⚠️ El **`brujula-display` del Curso 7 nunca se construyó** (19-sep-2026, ADR-067): su diseño lo pedía en la L2 para leer las reflexiones de cierre del Nivel 1, pero esta línea no tiene ese componente ni en el build ni en el esquema. El curso **publicado** lo sustituye por un `info-box` que le pide al adulto ir a buscarlas. **El documento de diseño ya está corregido** (v1.1, 19-sep-2026): describe lo que existe y deja anotado qué haría falta para portar el componente. Portarlo desde Desarrollo Institucional o Transversales es trabajo real —el renderizador de allí lee **un** curso y **un** módulo por caja, no seis— y sigue como **decisión abierta**.
